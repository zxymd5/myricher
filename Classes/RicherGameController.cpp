...
void RicherGameController::pickOnePlayerToGo()
{
	for (auto it = GameBaseScene::players_vector.begin(); it != GameBaseScene::players_vector.end(); it++)
	{
		...
		if (richerPlayer->getIsMyTurn())
		{
			...
			RouteNavigation::getInstance()->getPath(richerPlayer, randNumber,
													...
													GameBaseScene::tiledRowsCount,
													GameBaseScene::tiledColsCount);
			richerPlayer->startGo(RouteNavigation::getInstance()->getPathRows_vector(),
	...
	NotificationCenter::getInstance()->postNotification(RICHER_MSG, __String::createWithFormat("%d", MSG_GO_SHOW_TAG));
}
